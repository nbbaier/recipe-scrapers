...
import { FoodNetwork } from "@/scrapers/FoodNetwork";

describe("FoodNetwork", () => {
	let scraper: FoodNetwork;
	let html: string;

	beforeEach(() => {
		html = readFileSync("tests/test_data/foodnetwork.co.uk/foodnetwork.test.html", "utf8");
		scraper = new FoodNetwork(html, "/recipe/test");
	});

	it("should have correct host", () => {
		expect(FoodNetwork.host()).toBe("foodnetwork.co.uk");
	});
